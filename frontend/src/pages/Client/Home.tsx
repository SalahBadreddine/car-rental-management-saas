...
export default function ClientHome() {
  const { tenantId, isTenantDataLoading } = useAuth();
  

  const { data: featuredCars = [] } = useQuery({
    queryKey: ['featured-cars-client', tenantId],
    queryFn: () => carsApi.getFeaturedCars(3),
    enabled: !!tenantId,
  });
      ...
      <HeroBackground trace={traceCar} car={heroCar}>
        ...
        <div className="flex flex-wrap gap-4">
          <Button
            ...
            asChild
          >
            <Link to="/client/vehicles">Get your car today</Link>
          </Button>
          ...
          >
            {featuredCars.length > 0 ? (
             ...
             <div className="col-span-3 text-center text-white">
               No featured cars yet. Go to <Link to="/client/vehicles" className="text-primary underline">Vehicles</Link> and mark cars as "Best Deal" (star icon).
             </div>
            )}
          </div>
